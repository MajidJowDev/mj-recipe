...
public class DataLoaderMySQL implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent contextRefreshedEvent) {
        ...
        if (categoryRepository.count() == 0L){ //to make sure that these values will be added only if the table is empty
            log.debug("Loading Categories from MySQL");
            loadCategories();
        }

        if (unitOfMeasureRepository.count() == 0L){
            log.debug("Loading UOMs from MySQL");
            loadUom();
        }
    }
